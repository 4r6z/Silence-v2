...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y), IM_COL32(124, 133, 144, 255 * imgui_alpha), popup_open ? ImGuiDir_Up : ImGuiDir_Down, 1.0f);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        ...
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 1), grab_bb.Max + ImVec2(2, 1), IM_COL32(213, 125, 124, 255 * imgui_alpha), style.FrameRounding);
        window->DrawList->AddCircleFilled(grab_bb.GetCenter() + ImVec2(4, 0), 4.f, IM_COL32(255, 255, 255, 200 * imgui_alpha));
        ...
        if (hovered || value_changed)
        {
            ...
            ImGui::GetForegroundDrawList()->AddRectFilled(popup_bb.Min, popup_bb.Max, IM_COL32(22, 27, 27, 255 * imgui_alpha), style.PopupRounding);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, render_cursor ? GetColorU32(ImGuiCol_FrameBgActive) : hovered ? GetColorU32(ImGuiCol_FrameBgHovered) : GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    draw_list->AddCircleFilled(sv_cursor_pos, 3.f, IM_COL32_WHITE);
    draw_list->AddCircle(sv_cursor_pos, 3.f, ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Border)));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    auto bg_col = IM_COL32(59, 59, 59, 200 * imgui_alpha);
    auto bdr_col = IM_COL32(114, 114, 114, 200 * imgui_alpha);

    if (hovered && !selected)
        bdr_col = IM_COL32(139, 139, 139, 200 * imgui_alpha);
    else if (hovered && selected)
        bdr_col = IM_COL32(139, 139, 139, 100 * imgui_alpha);

    if (!selected) window->DrawList->AddRect(bb.Min + ImVec2(6, 0), bb.Min + ImVec2(6 + 12.f, 12.f), bdr_col, 1.f);

    if (selected && !hovered)
        bg_col = IM_COL32(213, 125, 124, 170 * imgui_alpha);
    else if (selected && hovered)
        bg_col = IM_COL32(213, 125, 124, 230 * imgui_alpha);

    window->DrawList->AddRectFilled(bb.Min + ImVec2(7, 1), bb.Min + ImVec2(6 + 11.f, 11.f), bg_col, 1.f);
