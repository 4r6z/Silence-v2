...
public:
    ...
    inline c_multi_listbox(const std::string_view& name, const std::string_view& label, std::vector<BYTE>* values, const std::vector<std::string>& items, const bool default_visibility = true) noexcept
    {
        ...
        this->m_type = framework_enums::element_type::MULTI_LISTBOX;
...
public:
    ...
    inline c_single_listbox(const std::string_view& name, const std::string_view& label, int* value, std::vector<std::string>* items, const bool default_visibility = true) noexcept
    {
        ...
        this->m_type = framework_enums::element_type::SINGLE_LISTBOX;
    ...
    virtual bool render()
    {
        ...
        if (const bool selected = ImGui::BeginListBox(std::string("##vis_" + m_label).c_str(), m_size)) {
            ...
            for (int i = 0; i < m_items->size(); i++) {
                ...
                if (ImGui::Selectable(m_items->at(i).c_str(), &is_selected, ImGuiSelectableFlags_SpanAvailWidth)) {
                    *m_selected = i;
                }
            }
    ...
    void set_value(int val) { *m_selected = val; }

    void set_items(const std::vector<std::string>& items) {m_items->clear(); m_items->insert(m_items->end(), items.begin(), items.end()); }
    void set_full_length(bool val) { m_full_length = val; }
