...
private:
    ...
    std::vector<c_binding*> get_all_binds() const
    {
        ...
        for (const auto& child : m_children)
            for (const auto& element : child->m_elements)
                if (element->m_type == framework_enums::element_type::KEYBIND)
                    tmp_vec.push_back(((c_keybind*)element)->get_bind());
                else if (element->m_type == framework_enums::element_type::GROUP)
        ...
        for (auto& element : m_elements)
        {
            ...
            if (element->get_visible()) [[likely]]
            {
                ...
                if (element->m_has_custom_position)
                {
                    ...
                    if (element->m_custom_position.x != 0.f)
                        ImGui::SetCursorPosX(element->m_custom_position.x);
                    
                    if (element->m_custom_position.y != 0.f)
                        ImGui::SetCursorPosY(element->m_custom_position.y);
                ...
                if (element->m_center_x)
                    ImGui::SetCursorPosX((ImGui::GetWindowWidth() - ImGui::CalcTextSize(element->m_label.c_str()).x) * 0.5f);
    ...
    void start_window_render() { ImGui::SetNextWindowSize(m_size * m_dpi_scale);  ImGui::Begin(m_name.c_str(), NULL, m_flags | ImGuiWindowFlags_NoSavedSettings); ImGui::RenderDropShadow(ImGui::my_texture, 40.f, 255); }
