...
private:
    ...
    void start_child_render(float dpi_scale) noexcept
    {
        ...
        m_child_drawn = ImGui::BeginChild(m_name.c_str(), m_size * dpi_scale, true, ImGuiWindowFlags_NoScrollbar);
        ...
        else if (hovered && ImGui::IsPopupOpen(nullptr, ImGuiPopupFlags_AnyPopup))
            m_animator.start_animation(true, true);

        m_animator.update_animation();

        //ImGui::GetForegroundDrawList()->AddRect(ImGui::GetWindowPos(), ImGui::GetWindowPos() + (m_size * dpi_scale), IM_COL32(213, 125, 124, ImGui::GetStyle().Alpha != 1.f ? 0 : std::clamp(static_cast<int>(m_animator.get_value() * 255.f), 0, 60)), ImGui::GetStyle().ChildRounding);

        if (m_has_header)
        {

            ImGui::SetCursorPosX(6 * dpi_scale);
            ImGui::TextGradiented(m_name.c_str(), IM_COL32(213, 125, 124, 255), IM_COL32(65, 23, 24, 255));
            ImGui::Spacing();
    ...
    void end_child_render() const noexcept
    {
        ...
        ImGui::EndChild();
        if (m_child_drawn) ImGui::EndChild();
...
public:
    ...
    void set_render_callback(const std::function<void(void)>& value) noexcept { m_render_callback = value; }
    ...
    {
        // Check what arguments are being passed
        type* allocated_element = new type(initializers...); // Check this line
